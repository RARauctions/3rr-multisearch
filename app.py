...
HEADERS = {"User-Agent": "RRAggregator/1.0"}
...
{% if q is not none %}
  ...
  {% for r in results %}
    <div class="grid">
      <div>{% if r.thumb %}<img src="{{r.thumb}}" loading="lazy">{% endif %}</div>
      <div>
...
def _extract_ids(html_text: str):
    ids = set(int(m.group(1)) for m in re.finditer(r"/ui/auctions/(\d+)", html_text))
    return sorted(ids)
...
def discover_auctions() -> list[int]:
    ...
    if ids:
        return ids
    # Fallback: render JS
    rendered = _render_js(INDEX_URL, timeout=40)
    return _extract_ids(rendered) if rendered else []

def parse_results(html_text: str, auction_id: int, query_url: str):
...
def fetch_results(aid: int, query: str):
    ...
    if items:
        return items
    # Fallback to JS render if static parse found nothing
    rendered = _render_js(qurl, timeout=40)
